...
def compute_delta_r(a, el, i, u):
    pl = compute_pl(a, el)
    delta_r = k2 / (2. * pl) * (1. - math.cos(i) * math.cos(i)) * math.cos(2. * u)
    return delta_r

def compute_delta_u(a, el, i, u):
    pl = compute_pl(a, el)
    delta_u = -k2 / (4. * pl * pl) * (7. * math.cos(i) * math.cos(i) - 1.) * math.sin(2. * u)
    return delta_u

def compute_delta_o(a, el, i, u):
    pl = compute_pl(a, el)
    delta_o = 3. * k2 * math.cos(i) * math.sin(2. * u) / (2. * pl * pl)
    return delta_o

def compute_delta_i(a, el, i, u):
    pl = compute_pl(a, el)
    delta_i = 3. * k2 * math.cos(i) * math.sin(i) * math.cos(2. * u) / (2. * pl * pl)
    return delta_i

def compute_delta_rdot(a, el, i, u, n):
    pl = compute_pl(a, el)
    delta_rdot = - k2 * n / pl * (1. - math.cos(i) * math.cos(i)) * math. sin(2. * u)
    return delta_rdot

def update_rk(a, el, i, u, r):
    pl = compute_pl(a, el)
    term1 = r * (1. - (3./2.) * k2 * math.sqrt(1. - el * el) * (3. * math.cos(i) * math.cos(i) - 1.) / (pl * pl))
    delta_r = compute_delta_r(a, el, i, u)
...
def compute_position(a, el, i, u, r, o):
    rk = update_rk(a, el, i, u, r)
    uk = update_uk(a, el, i, u)
    ok = update_ok(a, el, i, u, o)
    ik = update_ik(a, el, i, u)
    Uvec = U_vector(uk, ok, ik)
    r = rk * Uvec
    return r

def short_periodic_gravity_update(axN, ayN, E_plus_w, a, i, o):
    el = compute_eccentricity(axN, ayN)
    (eCosE, eSinE) = eTrigE(axN, ayN, E_plus_w)
    r = get_r(a, eCosE)
    u = compute_u(a, r, E_plus_w, axN, ayN, el, eSinE)

    pos = compute_position(a, el, i, u, r, o)
    return pos